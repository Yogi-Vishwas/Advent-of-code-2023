...
function getWinnings(filePath: PathLike) {
  function getHandType(hand: string): HandType {
    ...
    const values = Object.entries(freqMap).sort(([, a], [, b]) => a - b);
    if (freqMap["J"] > 0) {
      if (freqMap["J"] === 5) {
        freqMap["A"] = 5;
      } else if (values[values.length - 1][0] === "J") {
        freqMap[values[values.length - 2][0]] += freqMap["J"];
      } else {
        freqMap[values[values.length - 1][0]] += freqMap["J"];
      }
    }

    const val = Object.entries(freqMap)
      .sort(([, a], [, b]) => a - b)
      .reduce((acc, item) => {
        if (item[0] !== "J") {
          return acc + item[1];
