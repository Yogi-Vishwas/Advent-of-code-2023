...
async function getPoints(docPath: PathLike) {
  ...
  function getCards(cardNumber: number) {
    let num: number;
    let res = 1;
    let nCards: number;
    const matches = matchesMap[cardNumber] ?? 1;
    for (num = cardNumber + 1; num <= cardNumber + matches; ++num) {
      if (cardsMap[num]) {
        nCards = cardsMap[num];
      } else {
        nCards = getCards(num);
  ...
  for await (const line of rl) {
    ...
    matchesMap[cardNumber] = matches;
  }

  for (const key in matchesMap) {
    const res = getCards(+key);
    result += res;
  }
