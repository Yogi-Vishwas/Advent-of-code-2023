...
function calculateLeastHeat(heatMap: number[][], sourceVertex: Vertex) {
    ...
    while (queue.length) {
        ...
        if (currentRow === nRows - 1 && currentColumn === nColumns - 1) return currentKey;
        ...
        for (const direction of Object.keys(directionMap) as Directions[]) {
            ...
            if (currentDirection === direction && currentSteps === 3) continue;

            if (currentDirection === direction && currentSteps < 3)
                queue.insert({
